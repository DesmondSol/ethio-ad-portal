...
export const ReviewAndPay: React.FC<ReviewAndPayProps> = ({ setStep, campaign }) => {
  ...
  const videoPreviewUrl = campaign.adContent ? URL.createObjectURL(campaign.adContent) : '';
      ...
      <div className="bg-slate-800 p-8 rounded-lg shadow-xl space-y-6">
        ...
        <div>
            ...
            <div className="flex items-start space-x-4">
                <video src={videoPreviewUrl} className="w-48 rounded-lg shadow-lg" />
                <div>
                    <p className="font-semibold">{campaign.adContent.name}</p>
                    <button onClick={() => window.open(videoPreviewUrl)} className="text-emerald-400 hover:underline text-sm">Preview Fullscreen</button>
                </div>
            </div>
        </div>
