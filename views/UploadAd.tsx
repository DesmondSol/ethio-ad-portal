...
export const UploadAd: React.FC<UploadAdProps> = ({ setStep, updateCampaign, campaign }) => {
  ...
  const [filePreview, setFilePreview] = useState<string | null>(campaign.adContent ? URL.createObjectURL(campaign.adContent) : null);
  ...
  const handleFileChange = (files: FileList | null) => {
    if (files && files.length > 0) {
      ...
      if (!['video/mp4', 'video/quicktime', 'image/jpeg', 'image/png'].includes(file.type)) {
        alert("Invalid file format. Please upload MP4, MOV, JPG, or PNG.");
        return;
      }
      setAdFile(file);
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            >
              ...
              <p className="font-semibold text-white">Drag & drop your content here</p>
            ...
            <div className="text-sm text-slate-400 bg-slate-700/50 p-4 rounded-lg">
              ...
              <ul className="list-disc list-inside space-y-1">
                <li><span className="font-semibold text-slate-300">Format:</span> MP4, MOV, JPG, PNG</li>
                <li><span className="font-semibold text-slate-300">Duration:</span> 15 seconds (for videos)</li>
                <li><span className="font-semibold text-slate-300">Aspect Ratio:</span> 9:16 (Portrait)</li>
          ...
          <div>
            ...
            <div className="flex justify-center bg-slate-900/50 rounded-lg p-4">
              {adFile.type.startsWith('image/') ? (
                <img src={filePreview} alt="Ad preview" className="max-h-[60vh] object-contain rounded-lg shadow-lg" />
              ) : (
                <video src={filePreview} controls className="max-h-[60vh] rounded-lg shadow-lg"></video>
              )}
            </div>
            <p className="text-sm text-slate-400 mt-2 text-center">File: {adFile.name}</p>
          </div>
        )}
